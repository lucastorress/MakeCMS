...
if (isset($_GET['del']))
{
	fMessage('error', 'Voc� tem <b>certeza</b> que deseja deletar este item do cat�logo ? Isto n�o pode ser revertido.<br /><a href="index.php?_cmd=ot-cata-items&realdel=' . $_GET['del'] . '&sq=' . $_GET['sq'] . '">SIM, DELETAR</a> - ou - <a href="index.php?_cmd=ot-cata-items">CANCELAR</a>');
}
...
echo '<h1>Administrar itens do cat�logo</h1>';
...
if (mysql_num_rows($checkBlankItems) > 0)
{
	...
	echo '<p>';
	echo '<b>Aviso !</b> Se voc� deixar espa�os em branco, poder� comprometer o banco de dados:<br />';
	...
	while ($item = mysql_fetch_assoc($checkBlankItems))
	{
		...
		{
			echo '<li><i>Voc� est� editando o item (ID #' . $item['id'] . ').</i></li>';
		}
		else
		{
			echo '<li><a href="index.php?_cmd=ot-cata-items&edit=' . $item['id'] . '" target="_self">Item (ID #' . $item['id'] . ') na p�gina ' . $item['page_id'] . '</a> (or <a href="index.php?_cmd=ot-cata-items&del=' . $item['id'] . '">Deletar</a>)</li>';
		}
	}
...
{
	echo '<hr /><br />';
	echo '<small><b>Editando item do cat�logo</b></small>';
...
else
{
	...
	if (isset($_POST['search-query']))
	{
		...
		echo '<a href="index.php?_cmd=ot-cata-items&new"><b>Gerado novo item de cat�logo</b></a><br /><br />';
...
echo '<a href="index.php?_cmd=ot-cata-items&new"><b>Gerado novo item do cat�logo</b></a>';
